"""
build_clustering_control_v2.py

Supersedes build_clustering_control_v1.py. NAMED BUG in v1's Step 4:
`fetch_full_genome_gene_pool()` POSTed the BioMart XML query as form-encoded
data (`requests.post(..., data={"query": xml})`). Ensembl's martservice
endpoint returned 405 Method Not Allowed -- confirmed against the real
error, not guessed. BioMart's documented access pattern for this endpoint
is a GET request with the XML query passed as a URL query parameter (the
same pattern `wget`/`curl` examples in Ensembl's own BioMart docs use),
not a POST body. Steps 1-3 and 5 are unchanged from v1 -- they never
touched the network and already ran correctly against the real 373-gene
file. Only the request method/shape in Step 4 changes.
...
def fetch_full_genome_gene_pool():
    resp = requests.get(BIOMART_URL, params={"query": BIOMART_XML_QUERY}, timeout=120)
