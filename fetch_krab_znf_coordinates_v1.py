"""
fetch_krab_znf_coordinates_v1.py

Day 59 Block 4. Acquires genomic coordinates for the human KRAB-ZNF gene
set, as the input to Block 5's clustering-corrected control.

CANNOT BE RUN IN THIS SANDBOX: rest.genenames.org and rest.ensembl.org are
not on the sandbox's network allowlist. Same situation as Block 3's scripts
-- run this locally (VS Code Run button, zero configured arguments, per
project convention).

WHY THIS SCRIPT SELF-DISCOVERS THE HGNC FIELD NAME INSTEAD OF HARDCODING IT:
HGNC has renamed "gene family" to "gene group" in its own UI/docs before,
and I do not have live access to confirm which field name the REST API
currently uses. Rather than guess and risk a silent wrong-field query that
returns zero or garbage results, Step 1 below fetches ONE well-known
KRAB-ZNF gene (ZNF91) by symbol, prints every field name in the response,
and searches for whichever field name actually contains "family" or
"group" (case-insensitive). This is the schema-confirmation rule applied
to an API response instead of a CSV header -- same principle, different
data source.
...
OUTPUT_PATH = "data/krab_znf_coordinates_v1.csv"
...
def discover_family_field():
    ...
    candidate_fields = [k for k in doc.keys() if "famil" in k.lower() or "group" in k.lower()]
    if not candidate_fields:
        sys.exit(
            f"FATAL: no field name containing 'family' or 'group' found in the HGNC "
            f"record for {KNOWN_KRAB_ZNF_PROBE_GENE}. Actual fields: {sorted(doc.keys())}\n"
