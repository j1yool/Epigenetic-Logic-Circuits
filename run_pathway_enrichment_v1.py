...
def synthetic_self_test_background_filter() -> None:
    """
    Day 54 addition: this closes a real gap left by Day 53's fix. The
    original synthetic_self_test() only exercises the Enrichr positive
    control -- it never touches load_background_and_origin_groups(), which
    is exactly the function that had the silent NaN-vs-'NULL' bug. This
    test builds a small synthetic pair of gate-assignment TSVs with known,
    planted NaN and non-NaN gate_type rows and asserts the merged
    background contains exactly the expected gene_ids -- non-degenerate,
    known ground truth, per convention.
    """
    print("Running synthetic self-test: background NaN-filtering logic "
          "(load_background_and_origin_groups) against a small known-answer "
          "synthetic gate-assignment pair...")

    # Ground truth: genes A-F. gate_type is NaN for C (GM) and D (K562).
    # Expected surviving background after notna() filter + inner merge:
    # only genes present with a non-NaN gate_type in BOTH frames.
    gm_synthetic = pd.DataFrame({
        "gene_id": ["A", "B", "C", "D", "E"],
        "gate_type": ["SIMPLE_AND", "INCONSISTENT", np.nan, "REPRESSED", "BIVALENT"],
    })
    k562_synthetic = pd.DataFrame({
        "gene_id": ["A", "B", "C", "D", "F"],
        "gate_type": ["SIMPLE_OR", "COMPLEX", "REPRESSED", np.nan, "SIMPLE_AND"],
    })
    # Expected: A survives (non-NaN both sides), B survives (non-NaN both
    # sides), C is dropped (NaN in GM), D is dropped (NaN in K562), E and F
    # are dropped anyway (not present in both frames, unrelated to the
    # filter under test). Expected surviving set: {A, B}.
    expected_ids = {"A", "B"}

    with tempfile.TemporaryDirectory() as tmpdir:
        gm_path = Path(tmpdir) / "gm_synthetic.tsv"
        k562_path = Path(tmpdir) / "k562_synthetic.tsv"
        gm_synthetic.to_csv(gm_path, sep="\t", index=False)
        k562_synthetic.to_csv(k562_path, sep="\t", index=False)

        gm_raw = pd.read_csv(gm_path, sep="\t")
        k562_raw = pd.read_csv(k562_path, sep="\t")

        gm = gm_raw[gm_raw["gate_type"].notna()][["gene_id"]]
        k562 = k562_raw[k562_raw["gate_type"].notna()][["gene_id"]]
        result_ids = set(pd.merge(gm, k562, on="gene_id", how="inner")["gene_id"].tolist())

    if result_ids != expected_ids:
        fatal(f"Self-test FAILED: background NaN-filtering logic produced "
              f"{sorted(result_ids)}, expected {sorted(expected_ids)}. "
              f"This is the exact bug class that shipped on Day 53 (a filter "
              f"that matches nothing and silently keeps NaN rows) -- do not "
              f"proceed to real data until this reproduces the expected set.")

    print(f"Synthetic self-test PASSED: NaN-filtering logic correctly "
          f"excludes genes with missing gate_type in either frame "
...
def load_background_and_origin_groups(symbol_map: pd.Series):
    # Day 54 correction of the Day 53 fix (named, file/line-located): the
    # Day 53 version filtered on gate_type != "NULL", assuming missing gate
    # calls were stored as the literal string "NULL". Confirmed via
    # diagnostic on Day 54 that this is false -- missing calls are actual
    # NaN values (GM: 34,163 NaN rows; K562: 35,133 NaN rows, per
    # gate_type.value_counts()). In pandas, NaN != "NULL" evaluates to True,
    # so the Day 53 filter matched and kept every row, including all NaN
    # rows -- it silently did nothing, and the background count was
    # unchanged from the pre-fix value (confirmed: both runs returned
    # background_symbols == 42,771). The correct filter is gate_type.notna().
    gm_raw = pd.read_csv(GM_REGEN, sep="\t")
    if "gate_type" not in gm_raw.columns:
        fatal(f"{GM_REGEN} has no gate_type column. Found: {list(gm_raw.columns)}. "
              f"This file was previously confirmed to carry gate_type -- if that's "
              f"changed, stop and report the actual columns rather than guessing further.")
    k562_raw = pd.read_csv(K562_REGEN, sep="\t")
    if "gate_type" not in k562_raw.columns:
        fatal(f"{K562_REGEN} has no gate_type column. Found: {list(k562_raw.columns)}. "
              f"This file was previously confirmed to carry gate_type -- if that's "
              f"changed, stop and report the actual columns rather than guessing further.")
...
def main():
    synthetic_self_test_background_filter()
    synthetic_self_test()
